...
Otomobil::Otomobil(string _renk, string _model, int _beygirGucu, int _modelYili, int _KS)
constructor metodu gibi bir metod kullaniriz.*/
Otomobil::Otomobil(string _model, string _renk, int _beygirGucu, int _modelYili, int _KS) {

	model = _model;
	renk = _renk;
	beygirGucu = _beygirGucu;
	modelYili = _modelYili;
	pKapiSayisi = new int(_KS); // *pKapisayisi = _KS; seklinde de yazilabilir, ancak bu sekilde yazmak daha kisa ve daha kolaydir.

	cout << model << " uye degiskenine sahip olan objenin constructor'u cagrildi." << endl
		<< endl;
}

/* Destructor'un kullanim amaclarindan biri class'in icinde dinamik olarak olusturulmus o_
lan pointerları delete etmektir. */
Otomobil::~Otomobil() {
	cout << model << " uye degiskenine sahip olan objenin destructor'u cagrildi." << endl;
	delete pKapiSayisi; /* Contructor icinde dinamik olarak olusturulan pointer'ı destruc_
	tor icinde delete etmezsek memory leak olusur. */
}
...
void Otomobil::ruhsatBilgileriGoster() {
	...
	cout << "Model Yili: " << Otomobil::modelYili << endl;
